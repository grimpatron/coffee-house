...
function getElementValue(id: string) {
  ...
  if (element !== null) result = (element as HTMLInputElement).value;
  return result;
}

export function generateInterface(): HTMLDivElement {
  const div = document.createElement('div');
  div.className = 'garage-interface';
  div.innerHTML = `
    <input class="interface-text"   id="car-name-new"     type="text"   placeholder="Car model">
    <input class="interface-color"  id="car-color-new"    type="color"  value="#0000ff" />
    <input class="interface-btn"    id="btn-create"       type="button" value="Create" />
    <input class="interface-text"   id="car-name-change"  type="text"   disabled>
    <input class="interface-color"  id="car-color-change" type="color"  value="#0000ff" disabled/>
    <input class="interface-btn"    id="btn-update"       type="button" value="Update" />
    <input class="interface-btn"    id="btn-race"         type="button" value="Race" />
    <input class="interface-btn"    id="btn-reset"        type="button" value="Reset" />
    <input class="interface-btn"    id="btn-generate"     type="button" value="Generate car" />`;
  return div;
}
...
export function generateCarItem(newCar: Car): void {
  const garageUl = document.querySelector('.garage-list');
  const garageLi = document.createElement('li');
  const cnn = newCar.name;
  const ccn = newCar.color;
  garageLi.className = 'garage-item';
  garageLi.id = newCar.id;
  garageLi.innerHTML = `
    <div class="garage-item__controls">
      <input class="garage-item__btn" type="button" value="Select">
      <input class="garage-item__btn" type="button" value="Remove">
      <div class="garage-item__action-container">
        <input class="garage-item__action" type="button" value="A">
        <input class="garage-item__action" type="button" value="B">
      </div>
    </div>
    <div class="garage-car">
      <div class="garage-item__name">${cnn}</div>
      <div class="garage-item__color">${ccn}</div>
      <div class="garage-item__image">
...
export function saveCarToGarage() {
  const cnn = getElementValue('car-name-new');
  const ccn = getElementValue('car-color-new');
  if (cnn !== '' && cnn !== undefined && ccn !== undefined) {
    const userData = getStorageData();
    const newCar: Car = { name: cnn, color: ccn, id: generateUniqueID() };
    generateCarItem(newCar);
    userData.garage.list.push(newCar);
    saveUserData(userData);
  }
}

export function addEvent(id: string, eventType: string, callback: () => void) {
  const button = document.getElementById(id);
  if (button) button.addEventListener(eventType, callback);
}
